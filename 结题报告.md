...
本项目在之前课程项目的基础上，通过 TODO，进一步实现了 TODO 等优化，从而一方面可以用作易于部署、可用性高的“私人网盘”，另一方面也为用户群的扩展和服务推广留出了上升空间，提供了替代商业云存储的选择。
...
原先的文件夹分层不适合给关系型数据库，文件 meta 信息放数据库的结果就是列一次表就要查询一整个数据库。我们把文件和属性放在企业级 kv 数据库 redis 中，大大增强了 metadata 存取的速度。
...
- RSV1，RSV2，RSV3：各 1 bit。保留位，必须是 0。
...
本小组在考虑组员的兴趣并与刑凯老师进行充分的讨论之后，选择实现一个普适性强，P2P 传输，有潜力替代商业云存储，建立在互联网上的分布式文件系统。
...
1. [Erasure Code - EC 纠删码原理](https://blog.csdn.net/shelldon/article/details/54144730)
...
1. [译 Go 和 WebAssembly：在浏览器中运行 Go 程序](https://www.qichengzx.com/2019/01/01/go-in-the-browser.html)
...
1. [使用 Go 开发前端应用（三）](https://juejin.im/post/5eb2191df265da7bbf21a0f4)
